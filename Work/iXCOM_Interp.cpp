...
//по 3м точкам находим коэффициенты "a, b, c" квадратичной функции
void ComputeQuadrCoeff(double* Temp, double* Val, double* QuadrCoeff) //3х мерные вектора
{
...
//записываем коэффициеты для параболической интерполяции для каждой тройки
void RecordQuadrCoefAll(Par* sPar)
{
	RecordQuadrCoefAccBias(sPar);		// cмещения нулей акселерометров
	RecordQuadrCoefGyroBias(sPar);		// дрейф ДУС
	RecordQuadrCoefAccScales(sPar);		// масштабные коэффициенты акселерометров
	RecordQuadrCoefGyroScales(sPar);	// масштабные коэффициенты ДУС
	return;
}

// записываем коэффициеты для параболической интерполяции cмещений нулей акселерометров
void RecordQuadrCoefAccBias(Par* sPar)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		ComputeQuadrCoeff(Temp, Delta_F, QuadrCoeff); //вычисление коэф-тов "a, b, c"  функции y=ax^2+bx+c по 3м опорным точкам
		for (int j=0; j < 3; j++)
...
// записываем коэффициеты для параболической интерполяции дрейфов ДУС
void RecordQuadrCoefGyroBias(Par* sPar)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		ComputeQuadrCoeff(Temp, Nu, QuadrCoeff); //вычисление коэф-тов "a, b, c"  функции y=ax^2+bx+c по 3м опорным точкам
		for (int j=0; j < 3; j++)
			sPar->Clb_Database.InterpGyro.QuadrCoefBias[j+3*i]=QuadrCoeff[j];
	}
}

// записываем коэффициеты для параболической интерполяции масштабных коэффициентов акселерометров
void RecordQuadrCoefAccScales(Par* sPar)
{
	...
	for (int i = 0; i < 9; i++)
	{
		...
		ComputeQuadrCoeff(Temp, Kf, QuadrCoeff); //вычисление коэф-тов "a, b, c"  функции y=ax^2+bx+c по 3м опорным точкам
		for (int j=0; j < 3; j++)
			sPar->Clb_Database.InterpAcc.QuadrCoefScales[j+3*i]=QuadrCoeff[j];
	}
}

// записываем коэффициеты для параболической интерполяции масштабных коэффициентов ДУС
void RecordQuadrCoefGyroScales(Par* sPar)
{
	...
	for (int i = 0; i < 9; i++)
	{
		...
		ComputeQuadrCoeff(Temp, Kw, QuadrCoeff); //вычисление коэф-тов "a, b, c"  функции y=ax^2+bx+c по 3м опорным точкам
		for (int j=0; j < 3; j++)
			sPar->Clb_Database.InterpGyro.QuadrCoefScales[j+3*i]=QuadrCoeff[j];
	}
}

//интерполируем параболой, эстраполируем касательными к параболе (вне опорных точек)
...
// расчет всех текущих оценок, зависящих от температуры
void ComputeCurAll (Par* sPar) 
{
	ComputeCurAccBias(sPar);		// расчет текущих оценок смещения нулей акселерометров
	ComputeCurGyroBias(sPar);		// расчет текущих оценок дрейфов ДУС
	ComputeCurAccScales(sPar);		// расчет текущих оценок мастабных коэф-тов акселерометров
	ComputeCurGyroScales(sPar);		// расчет текущих оценок мастабных коэф-тов гироскопов
	return;
}

// расчет текущих оценок смещения нулей акселерометров
...
// расчет текущих оценок дрейфов ДУС
...
// расчет текущих оценок мастабных коэф-тов акселерометров
...
// расчет текущих оценок мастабных коэф-тов гироскопов
...
// запись текущих оценок в текстовый файл
...
// запись калибровочной таблицы в файл
